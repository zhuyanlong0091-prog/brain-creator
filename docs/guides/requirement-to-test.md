...
After a confirmed System Brain exists, use `explorationMode=incremental` for a targeted refresh. The run keeps the confirmed snapshot as its baseline and records only stale, low-confidence, or behavior-uncovered page routes as targets. Use `bc_review target=system-brain` to inspect stable page identities and `bc_review target=system-exploration` to inspect the selected scope; a full exploration is still the fallback when no confirmed baseline exists.
