...
The archive contains `manifest.json` and available evidence files. It does not include the repository, secret material, browser storage state, or unrelated workspace files. Host-agent Generator/Healer submissions are checked before they are accepted, and ZIP export performs a second scan. Both stages scan known protected credential values and high-confidence credential patterns such as private keys, JWTs, Bearer tokens, and literal password/token/cookie fields. Missing evidence is listed in the manifest instead of silently omitted.
