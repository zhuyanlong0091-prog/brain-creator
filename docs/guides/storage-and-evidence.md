...
The archive contains `manifest.json` and available evidence files. It does not include the repository, secret material, browser storage state, or unrelated workspace files. Before writing the ZIP, Brain Creator scans both known protected credential values and high-confidence credential patterns such as private keys, JWTs, Bearer tokens, and literal password/token/cookie fields. Missing evidence is listed in the manifest instead of silently omitted.
