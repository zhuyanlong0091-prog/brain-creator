...
Requirement sources are parsed into an ordered block AST before semantic analysis. Headings retain their level, Markdown/HTML/DOCX tables retain headers and rows, images retain their reference, and each block carries source anchors. Feishu direct and host-provided packages are normalized with the same fallback anchors; `.larkenterprise.com` is treated as a supported Feishu host. `generate-analysis` defaults to the host-agent Harness when the provider is omitted. Builtin analysis remains an offline compatibility path, but structured or visual sources are preview-only under an explicit builtin request.
...
Page identities are persisted separately from capture records. Query strings, dynamic record IDs, and a renamed page label do not create a new semantic page; the identity keeps its revision, latest PageModel, confidence, and confirmation history. A full exploration remains available for onboarding. Later calls to `explore-system` can set `explorationMode=incremental`: Brain Creator targets only pages whose model revision changed, whose evidence is low-confidence, or whose behavior has not been covered by the confirmed snapshot. The selected scope and skipped page count are stored on the exploration for review.
