...
If the host connector already downloaded a Feishu attachment, pass it in the same `analyze-attachments` call with `hostDownloadedAttachments: [{ attachmentId, localPath, contentHash?, mimeType? }]`. The path must be inside `BRAIN_CREATOR_WORKSPACE`; Brain Creator reads and hashes the file, copies it into the controlled source archive, and then returns the normal `needs-host-vision` request. Never pass a time-limited Feishu URL or an access token. A verified host download can recover the attachment Gap, but visual analysis remains draft until the host submits and the user confirms it.
