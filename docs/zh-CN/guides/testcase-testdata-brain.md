...
当生产者用例已经持有活动 Lease 时，消费者用例会继承一个只读绑定，直接指向这条明确的实体 Lease。只有声明的依赖边、唯一的当前生产者用例同时成立时才能复用；其他用例即使文本相同也不能满足依赖。生产者尚未执行或绑定存在歧义时，数据准备会保持阻塞或要求宿主补充数据。
...
每个 `BusinessScenario` 在编译用例前都会获得数据计划。计划记录选中的数据画像、稳定业务实体引用、画像之间的依赖，以及三种准备状态：

- `ready`：已有实体或确定性字段值可直接使用；
- `creatable`：需要在批准范围内创建数据，并执行清理策略；
- `blocked`：缺少数据画像、实体引用、依赖或清理策略。

按目标系统评估时，数据计划会带有系统上下文，不能被另一个系统复用。通过 `bc_review target=testdata` 查看计划和生命周期证据。实体会记录 lookup、create、transition、verify、cleanup 以及宿主 Agent 观察等事件，并保留 Provider 和来源引用，方便复盘。
