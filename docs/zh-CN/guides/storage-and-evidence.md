...
归档包含 `manifest.json` 和可用证据文件，不包含仓库、密钥、浏览器 storage state 或其他无关工作区文件。写入 ZIP 前，Brain Creator 除匹配当前受保护凭据值外，还会扫描私钥、JWT、Bearer Token，以及明文 password/token/cookie 字段等高置信凭据模式。缺失证据会列在 manifest 中，不会被静默忽略。
