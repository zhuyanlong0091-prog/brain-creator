...
| D8 | Screenshots had no business meaning | partial | PR C | Evidence now carries target semantic, input value, page model, locator point, data profile and source references into the static report with secret redaction; structured Reporter actual values now bind to assertion steps by `stepId` with single-assertion compatibility; richer visual annotations remain |
