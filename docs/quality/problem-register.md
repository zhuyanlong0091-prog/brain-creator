...
| B1 | SPA exploration missed navigation transitions | partial | PR E | Link navigation and observed safe-interaction URL changes now create queued targets and System Brain navigation edges; surface allowlist filtering and control-value state evidence are enforced, while full remount and cross-surface recovery remain |
