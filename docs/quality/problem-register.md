...
| A8 | Upgrade and migration had no safe path | resolved | PR B/PR F | Schema 16 migration creates a timestamped backup; schema 17 validation, index rebuild, complete Suite export, artifact dry-run/apply/hash validation/rollback, and explicit retention are covered |
