...
| D: Continuity and recovery | Partial | Existing recovery plus unified `executionTasks` projection in `bc_status`, including current case/step, page, wait reason, related data/agent task and next action; write-action lifecycle now records planned/sent/confirmed/reconciled phases, blocks repeat sends after interruption, and supports an injected automatic postcondition verifier; document suites now persist and incrementally rewrite a searchable offline HTML report | Target-specific verifier wiring, browser/process interruption replay, and real-system continuity |
...
- Fresh-context persistence coverage proves an unresolved sent action remains waiting and no new test case is dispatched. Injected-verifier coverage proves confirmed and unavailable automatic reconciliation paths; target-specific browser/process replay remains unmeasured.
...
- This closes the in-process guard and audit contract. An optional `ExecutionPostconditionVerifier` can now query a target-specific postcondition during reconciliation; `autoVerify=true` records the returned evidence only when the verifier says `confirmed`. Without a configured verifier, or when verification is unavailable/not confirmed, the action remains `reconciliation-required`. A real-system adapter and browser/process interruption end-to-end test are still outstanding.
