...
12. Inspect `stateActions` after path planning. Only `unique` or `not-required` may continue. Present state candidates when ambiguous and collect missing input or locator evidence when blocked. Treat all domain scenarios as data: never add product-specific branches to the planner.
