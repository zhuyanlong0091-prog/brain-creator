...
| Explore stateful or multi-role behavior | `bc_prepare action=create-exploration-plan`, preview/confirm approval, then `start-exploration-plan` and `submit-exploration-result` | One plan approval; test/staging allowlist, verified roles, write/time budgets, evidence, and cleanup are mandatory |
