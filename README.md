...
默认运行数据使用 `.brain-creator/store/` 下的 schema 20 分片仓库。schema 19 和旧的 `.brain-creator/local-assets.json` 会先备份再迁移；迁移不会把历史用例自动标记为 `verified` 或 `trusted`。`BRAIN_CREATOR_STORE_DIR` 可指定分片仓库位置。
...
Runtime state is stored by default in the schema 20 sharded repository under `.brain-creator/store/`. Schema 19 and legacy `.brain-creator/local-assets.json` stores are backed up before migration; legacy cases are not automatically marked `verified` or `trusted`. Set `BRAIN_CREATOR_STORE_DIR` to choose another shard directory.
