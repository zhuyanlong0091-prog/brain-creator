...
export async function generateSeedFile(input: GenerateSeedFileInput) {
  ...
  const content = [
    ...
    ``,
    `export const bc = {`,
    `  async step(stepId: string, page: { screenshot: (options: { path: string; fullPage?: boolean }) => Promise<unknown>; on: (event: string, listener: (...args: any[]) => void) => void; off: (event: string, listener: (...args: any[]) => void) => void }, action: () => Promise<void>) {`,
    ...
    `        await base.info().attach(\`brain-creator-runtime-\${stepId}\`, { body: JSON.stringify({ consoleErrors, networkFailures }), contentType: "application/json" });`,
