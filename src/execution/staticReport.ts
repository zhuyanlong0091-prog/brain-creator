...
<h2>Steps</h2><table id="steps"><thead><tr><th>#</th><th>Action</th><th>Instruction</th><th>Expected</th><th>Actual</th><th>Status</th><th>Screenshot</th></tr></thead><tbody>${evidence.steps.map((step) => `<tr class="searchable-row"><td>${step.order}</td><td>${escapeHtml(step.action)}</td><td>${escapeHtml(step.instruction)}</td><td>${escapeHtml(step.expected ?? "")}</td><td>${escapeHtml(step.actual ?? "")}</td><td class="${step.assertionStatus}">${escapeHtml(step.assertionStatus)}</td><td>${escapeHtml(step.screenshotPath ?? "")}</td></tr>`).join("")}</tbody></table>
