...
14. Call `prepare-execution confirm=false` and present every Requirement, System, Auth, Path, State, Data, Gap, and Cleanup check. Confirm only a ready draft. Identical semantic state reuses the immutable snapshot; blocked or needs-confirmation results cannot start Generator.
15. Preview with `bc_run mode=requirement-suite confirm=false`; execute only after confirmation with `bc_run mode=requirement-suite confirm=true`. The Suite repeats Preflight and binds the ready ExecutionPlan to Generator and ExecutionEvidence. After terminal execution, call `prepare-test-data` again and complete any returned cleanup or reacquisition task with evidence.
...
Execution Preflight is deterministic and read-only until confirmation. It creates a semantic hash over approved requirement state, system binding, optional explicit auth, executable steps, path/state plans, data leases, open Gaps, and cleanup obligations. Persist only `verdict=ready`; never bypass a blocked check with a lower-level execution tool. Keep secret material as references.
