...
12. Inspect `testDataPlan`. Present generated candidates during suite preview. For an existing reference, let the host Agent perform the approved lookup or creation, then call `bc_prepare action=resolve-test-data confirm=true` with the reference and non-secret value. Never resolve duplicate fields, missing dependencies, or cycles by inventing a value. Never copy a secret into an executable step.
13. Preview with `bc_run mode=requirement-suite confirm=false`; execute only after confirmation with `bc_run mode=requirement-suite confirm=true`.
14. Use `bc_review target=requirement-eval-accuracy`, `bc_review target=system-brain`, and `bc_review target=system-exploration` alongside evidence, BugReport, and Gap reviews.
...
Test-data planning is deterministic and TestIntent-scoped. `fixed`, `generated`, and `unique` values may be proposed for user confirmation; `existing-reference` requires an explicit reuse/create result from the host Agent; `runtime-captured` requires declared dependencies; `secret-reference` stays a reference. Structural graph failures remain Gaps.
