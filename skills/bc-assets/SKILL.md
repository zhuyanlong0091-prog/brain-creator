...
5. Call `bc_list_gaps` with `projectId` and optional `status` when the user asks what is blocked.
6. Call `bc_resolve_gap` only after the user confirms a gap has been handled.
7. Call `bc_search_assets` with `projectId` and a short query for broad asset lookup.
8. Keep results scoped to the current system.
