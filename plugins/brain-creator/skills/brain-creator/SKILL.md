...
| Approve a baseline without system onboarding | `bc_prepare action=approve-baseline confirm=true` | Compatibility path; explicit user confirmation required |
...
10. Present the complete Requirement baseline, unresolved questions, roles, routes, writes, duration, and cleanup policy. Call `approve-onboarding-plan confirm=true` only after one explicit user approval. This atomically approves the RequirementSet and linked ExplorationPlan; a failed preflight or changed baseline must leave both draft, and changed scope requires a new plan. Start with `start-onboarding-plan`, execute only the returned work package, and return action evidence plus one `taskEvidence` entry for every requirement question and requested evidence label with `submit-exploration-result`. Use `bc_review target=onboarding-plan` and `bc_status` to recover the plan. The older separate `approve-baseline` and ExplorationPlan flow remains a compatibility path.
