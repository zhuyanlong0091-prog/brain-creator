...
15. Preview with `bc_run mode=requirement-suite confirm=false`; execute only after confirmation with `bc_run mode=requirement-suite confirm=true`. Confirmation preflights every candidate and creates one ordered RequirementSuiteRun. Generator, Seed, and ExecutionEvidence consume the frozen plan. A stale hash at initial execution or `bc_submit_agent_output` must stop without task-submission side effects and return to `prepare-execution`.
16. Submit each Host Agent result normally; Brain Creator schedules the next queued case. Business failures continue after Bug creation. Technical blockers stop. Never bypass them automatically: use `resume=true, continueOnBlocked=true` only after the user reviews and accepts skipping the blocked case. Repeated confirmation must reuse the pending task.
