...
3. If the source has attachments, call `bc_prepare action=analyze-attachments`. Use the host multimodal capability for every returned local path, submit schema-valid visual analysis, present the draft, and confirm it only after explicit user approval. If a host connector already downloaded an attachment, pass `hostDownloadedAttachments: [{ attachmentId, localPath, contentHash?, mimeType? }]`; the path must be inside the Brain Creator workspace, and Brain Creator will hash and copy it before recognition. Never pass a time-limited URL or token, and never create an attachment Gap before download or recognition retries are attempted.
